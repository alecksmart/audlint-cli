...
| `bin/cover_album.sh` | Standardize one album to a single player-safe `cover.jpg`; rewrites one consistent embedded cover across FLAC, MP3, M4A, Ogg, and Opus tracks, using `vorbiscomment` for Ogg/Vorbis and `opustags` for Opus, manual runs preserve extra cover-like sidecars unless `--cleanup-extra-sidecars` is passed, and `--fetch-missing-art` can pull missing art from MusicBrainz / Cover Art Archive |
