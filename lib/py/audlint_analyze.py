...
def usage() -> str:
    return (
        ...
        "  audlint_analyze.py analyze <headroom_hz> <thresh_rel_db> <window_sec> <max_windows> <file...>"
    )
...
def ffprobe_audio_stream_meta(path: str) -> dict[str, str]:
    ...
    proc = subprocess.run(
        [
            ...
            "a:0",
            "-show_entries",
            "stream=codec_name,sample_rate,bits_per_raw_sample,bits_per_sample,sample_fmt",
            "-of",
...
def audio_meta(path: str) -> TrackMeta:
    ...
    return TrackMeta(sr=sr, dur=dur, bits=bits, prefer_ffmpeg_first=prefer_ffmpeg_decode(path, soxi_dur))
...
def decode_window(path: str, sr: float, t0: float, window_sec: float, prefer_ffmpeg_first: bool = False) -> bytes:
    ...
    sox_cmd = ["sox", path, "-t", "f32", "-c", "1", "-r", str(int(sr)), "-", "trim", f"{t0}", f"{window_sec}"]
...
def analyze_cutoff(
    ...
    cutoffs: list[float] = []
    for t0 in starts:
        raw = decode_window(path, sr, t0, window_sec, prefer_ffmpeg_first=prefer_ffmpeg_first)
        if not raw:
            continue
...
def map_to_target_sr(cutoff_hz: float | None, sr_in: float, headroom_hz: int) -> int:
    if cutoff_hz is None:
        return int(sr_in)
    effective = max(0.0, cutoff_hz + headroom_hz)
    if effective <= 22050:
        target = 44100
    elif effective <= 24000:
        target = 48000
    elif effective <= 48000:
        target = 96000
    else:
        target = 192000
    return int(min(target, sr_in))
...
def cmd_analyze(argv: list[str]) -> int:
    ...
    for path in files:
        ...
        sr, _dur, cutoff = analyze_cutoff(path, window_sec, max_windows, thresh_rel_db, track_meta)
        if sr is None:
            continue
        tgt_sr = map_to_target_sr(cutoff, sr, headroom_hz)
        tracks.append(
            {
                "file": path,
